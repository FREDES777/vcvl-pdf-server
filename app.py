...
def desenhar_rodape():
    ...
    c.drawRightString(PW - M, 7*mm, 'Pagina ' + str(c.getPageNumber()))
...
# ── COMBINACOES PREMIADAS (lista detalhada) ──────────────────────────────────
if premiadas:
    if y < 40*mm:
        y = nova_pagina()
    c.setFillColor(COR_VERDE)
    ...
    c.setFont(_font_bold, 9)
    c.setFillColor(COR_BRANCO)
    c.drawString(M + 3*mm, y - 5.5*mm, 'Combinacoes Premiadas -- Detalhamento')
    y -= 8*mm

    for i, p in enumerate(premiadas):
        ...
        if y < 22*mm:
            ...
            c.setFont(_font_bold, 9)
            c.setFillColor(COR_BRANCO)
            c.drawString(M + 3*mm, y - 5.5*mm, 'Combinacoes Premiadas -- Detalhamento (continuacao)')
            y -= 8*mm
        ...
        c.setFillColor(COR_VERDE)
        c.drawString(M + 3*mm, y - 4.8*mm, '#' + str(idx_p))

        c.setFont(_font_bold, 8)
        c.setFillColor(COR_OURO if acertos_p >= 15 else COR_TEXTO)
        c.drawString(M + 18*mm, y - 4.8*mm, str(acertos_p) + ' acertos')

        c.setFont(_font, 8)
        c.setFillColor(COR_TEXTO)
        c.drawString(M + 45*mm, y - 4.8*mm, nums_txt)
...
agora      = data['agora']
detalhado  = data.get('detalhado', False)
...
def nova_pagina(primeira=False):
    ...
    c.setFont(_font, 8)
    c.setFillColor(colors.HexColor('#c8e6c9'))
    c.drawRightString(PW - MARGIN, PH - 17*mm, 'Gerado em: ' + agora)
    c.drawRightString(PW - MARGIN, PH - 23*mm, periodo)
...
for h in itens:
    ...
    # ── Combinacoes premiadas deste concurso (modo detalhado) ────────────────
    premiadas_h = h.get('premiadas') or []
    if detalhado and premiadas_h:
        if y < 26*mm:
            rodape(pag_atual[0], '?')
            pag_atual[0] += 1
            y = nova_pagina()
        ...
        c.setFillColor(COR_VERDE)
        c.drawString(MARGIN + 3*mm, y - 4*mm, 'Combinacoes Premiadas -- Concurso N. ' + str(h.get('num_concurso', '')) + '  ·  Sorteio: ' + sorteados_str)
        y -= 6*mm

        for pi, p in enumerate(premiadas_h):
            if y < 18*mm:
                rodape(pag_atual[0], '?')
                pag_atual[0] += 1
                y = nova_pagina()
                ...
                c.setFont(_font_bold, 7.5)
                c.setFillColor(COR_VERDE)
                c.drawString(MARGIN + 3*mm, y - 4*mm, 'Combinacoes Premiadas -- Concurso N. ' + str(h.get('num_concurso', '')) + ' (continuacao)')
                y -= 6*mm

            acertos_p = p.get('acertos', 0)
            numeros_p = p.get('numeros', [])
            idx_p = p.get('idx', pi + 1)
            nums_txt = '  '.join(str(n).zfill(2) for n in numeros_p)

            bg_p = COR_BRANCO if pi % 2 == 0 else COR_CINZA
            c.setFillColor(bg_p)
            c.rect(MARGIN, y - 5.5*mm, CONTENT_W, 5.5*mm, fill=1, stroke=0)

            c.setFont(_font_bold, 7)
            c.setFillColor(COR_VERDE)
            c.drawString(MARGIN + 3*mm, y - 3.8*mm, '#' + str(idx_p))

            c.setFont(_font_bold, 7)
            c.setFillColor(COR_TEXTO)
            c.drawString(MARGIN + 16*mm, y - 3.8*mm, str(acertos_p) + ' acertos')

            c.setFont(_font, 7)
            c.setFillColor(COR_TEXTO)
            c.drawString(MARGIN + 38*mm, y - 3.8*mm, nums_txt)
